...
const readPresentationIndex = function readPresentationIndex(value) {
  ...
  const namespaceSvgMarkup = function namespaceSvgMarkup(markup, namespace) {
  if (typeof markup !== "string" || !markup.includes("<svg")) return markup;
  const prefix =
    String(namespace || "svg")
      .replace(/[^a-z0-9_-]+/gi, "-")
      .replace(/^-+|-+$/g, "") || "svg";
  const escapePattern = (value) =>
    String(value).replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  const authoredIds = [];
  const authoredIdByCurrentId = new Map();
  markup.replace(/<[a-z][^>]*>/gi, (tag) => {
    const id = tag.match(/\sid\s*=\s*(["'])([^"']+)\1/i)?.[2];
    if (!id) return tag;
    const sourceId = tag.match(
      /\sdata-scrollstudio-source-id\s*=\s*(["'])([^"']+)\1/i
    )?.[2];
    authoredIdByCurrentId.set(id, sourceId || id);
    if (!authoredIds.includes(id)) authoredIds.push(id);
    return tag;
  });
  if (authoredIds.length === 0) return markup;

  const idMap = new Map(
    authoredIds.map((id, index) => [id, `${prefix}-${index + 1}`])
  );
  let result = markup.replace(
    /(\s)id\s*=\s*(["'])([^"']+)\2/gi,
    (match, whitespace, quote, id, offset, source) => {
      ...
      const hasSourceId = /\sdata-scrollstudio-source-id\s*=/i.test(tag);
      const sourceAttribute = hasSourceId
        ? ""
        : ` data-scrollstudio-source-id=${quote}${authoredIdByCurrentId.get(id) || id}${quote}`;
      return `${whitespace}id=${quote}${nextId}${quote}${sourceAttribute}`;
    }
  );

  for (const [authoredId, namespacedId] of idMap) {
    const escapedId = escapePattern(authoredId);
    result = result
      .replace(
        new RegExp(`(\\b(?:href|xlink:href)\\s*=\\s*["'])#${escapedId}(["'])`, "gi"),
        `$1#${namespacedId}$2`
      )
      .replace(
        new RegExp(`url\\(\\s*(["']?)#${escapedId}\\1\\s*\\)`, "gi"),
        `url(#${namespacedId})`
      )
      .replace(
        new RegExp(`(\\b(?:begin|end)\\s*=\\s*["'][^"']*)\\b${escapedId}(?=\\.)`, "gi"),
        `$1${namespacedId}`
      );
  }
  ...
  result = result.replace(
    ...
    (match, attribute, quote, value) => {
      ...
      return `${attribute}=${quote}${ids.join(" ")}${quote}`;
    }
  );

  return result.replace(/<style\b[^>]*>([\s\S]*?)<\/style>/gi, (block, css) => {
    const rewrittenCss = css.replace(/([^{}]+)\{/g, (opening, prelude) => {
      if (prelude.trimStart().startsWith("@")) return opening;
      let selector = prelude;
      for (const [authoredId, namespacedId] of idMap) {
        selector = selector.replace(
          new RegExp(`#${escapePattern(authoredId)}(?![a-z0-9_-])`, "gi"),
          `#${namespacedId}`
        );
      }
      return `${selector}{`;
    });
    return block.replace(css, rewrittenCss);
  });
};
  const getSvgAuthoredId = function getSvgAuthoredId(element) {
  return element?.dataset?.scrollstudioSourceId || element?.id || "";
};
  let svgRuntimeInstanceSequence = 0;
  const svgRuntimeNamespaceFor = function svgRuntimeNamespaceFor(host) {
  if (!host.__scrollstudioSvgNamespace) {
    svgRuntimeInstanceSequence += 1;
    host.__scrollstudioSvgNamespace = `scrollstudio-runtime-svg-${svgRuntimeInstanceSequence}`;
  }
  return host.__scrollstudioSvgNamespace;
};
  ...
  const parseSvgNumbers = function parseSvgNumbers(value) {
  return String(value ?? "")
    .trim()
    .split(/[\s,]+/)
    .map(Number)
  ...
  const readSvgAuthoredMatrix = function readSvgAuthoredMatrix(element) {
  try {
    const consolidated = element?.transform?.baseVal?.consolidate?.();
    return consolidated?.matrix
      ? svgDomMatrixValues(consolidated.matrix)
      : identitySvgMatrix();
  } catch {
    return identitySvgMatrix();
  }
};
  const readSvgNumber = function readSvgNumber(element, name, fallback = 0) {
  const value = Number.parseFloat(element?.getAttribute?.(name) ?? "");
  return Number.isFinite(value) ? value : fallback;
};
  const createNestedSvgViewportMatrix = function createNestedSvgViewportMatrix(element) {
  const x = readSvgNumber(element, "x");
  const y = readSvgNumber(element, "y");
  const viewBox = parseSvgNumbers(element?.getAttribute?.("viewBox"));
  ...
  if (preserve.startsWith("none")) {
    ...
    return [scaleX, 0, 0, scaleY, x - minX * scaleX, y - minY * scaleY];
  }
  ...
  const alignX = alignment.includes("xMax")
    ? remainingX
    : alignment.includes("xMid")
      ? remainingX / 2
      : 0;
  const alignY = alignment.includes("YMax")
    ? remainingY
  ...
  const readSvgNodeLocalMatrix = function readSvgNodeLocalMatrix(element, rootSvg) {
  if (!element || element === rootSvg) return identitySvgMatrix();
  const authored = readSvgAuthoredMatrix(element);
  if (String(element.tagName ?? "").toLowerCase() !== "svg") {
    return authored;
  }
  return multiplySvgMatrices(authored, createNestedSvgViewportMatrix(element));
};
  const getSvgElementToRootMatrix = function getSvgElementToRootMatrix(element, rootSvg) {
  let matrix = identitySvgMatrix();
  let current = element;
  while (current && current !== rootSvg) {
    matrix = multiplySvgMatrices(
      readSvgNodeLocalMatrix(current, rootSvg),
      matrix
    );
  ...
  const parentMatrix = getSvgElementToRootMatrix(element?.parentNode, svg);
  ...
  const readSvgPrimitiveBounds = function readSvgPrimitiveBounds(element) {
  const tagName = String(element?.tagName ?? "").toLowerCase();
  if (tagName === "image" || tagName === "rect") {
    const width = readSvgNumber(element, "width", Number.NaN);
    const height = readSvgNumber(element, "height", Number.NaN);
    if (width >= 0 && height >= 0) {
      return {
  ...
  if (tagName === "circle") {
    const radius = readSvgNumber(element, "r", Number.NaN);
    if (radius >= 0) {
      const centerX = readSvgNumber(element, "cx");
      const centerY = readSvgNumber(element, "cy");
      return {
        x: centerX - radius,
        y: centerY - radius,
        width: radius * 2,
  ...
  if (tagName === "ellipse") {
    const radiusX = readSvgNumber(element, "rx", Number.NaN);
    const radiusY = readSvgNumber(element, "ry", Number.NaN);
  ...
  if (geometryLayer) {
    geometryLayer.style.transformBox = "";
    geometryLayer.style.transformOrigin = "";
    geometryLayer.style.transform = "";
    geometryLayer.removeAttribute?.("transform");
  }
};
  ...
  const layerId = getSvgAuthoredId(group);
  const existingGeometry = group.parentElement;
  if (existingGeometry?.dataset?.scrollyLayerGeometry === layerId) {
    const existingMotion = existingGeometry.parentElement;
    if (existingMotion?.dataset?.scrollyLayerMotion === layerId) {
  ...
  if (existingMotion?.dataset?.scrollyLayerMotion === layerId) {
    ...
    geometry.dataset.scrollyLayerGeometry = layerId;
  ...
  motion.dataset.scrollyLayerMotion = layerId;
  geometry.dataset.scrollyLayerGeometry = layerId;
  ...
  try {
    const bounds = readSvgPrimitiveBounds(group) ?? group.getBBox?.();
    if (!bounds) return fallback;
    const localPoint = {
      x: bounds.x + bounds.width * (pivot.xPercent / 100),
      y: bounds.y + bounds.height * (pivot.yPercent / 100)
    };
    const groupToRootMatrix = getSvgElementToRootMatrix(group, svg);
  ...
  const resolveSvgLayerParentMatrices = function resolveSvgLayerParentMatrices({
  groupsById,
  states,
  pivotPoints,
  parents,
  ...
  if (!(sourceSize.width > 0 && sourceSize.height > 0)) return new Map();
  ...
  Object.keys(deviceParents).forEach((childId) => inheritedMatrix(childId));
  return inheritedMatrices;
};
  ...
  const groupsById = new Map(
    Array.from(host.querySelectorAll("[id]")).map((group) => [
      getSvgAuthoredId(group),
      group
    ])
  );
  ...
  preparedLayerIds.forEach((layerId) => {
    ...
    if (inheritance) inheritanceById.set(layerId, inheritance);
  });
  ...
  const inheritedMatrices = resolveSvgLayerParentMatrices({
    groupsById,
    states,
    pivotPoints,
    parents,
  ...
  wrappersById.forEach((wrappers, layerId) => {
    ...
    const inheritance = inheritanceById.get(layerId);
    ...
    motionGroup.style.opacity =
      deviceState.opacity < 1
        ...
        : "";
    motionGroup.style.transform = "";
    motionGroup.removeAttribute?.("transform");
    geometryGroup.style.transform = "";
    geometryGroup.removeAttribute?.("transform");
    const ownMatrix = createSvgLayerMatrix(
      transform,
      pivotPoints.get(layerId),
      sourceSize
    );
    const inheritedMatrix = inheritedMatrices.get(layerId) ?? identitySvgMatrix();
    const rootMatrix = multiplySvgMatrices(inheritedMatrix, ownMatrix);
    localMatrices.set(
      layerId,
      convertRootMatrixToSvgElementSpace(inheritance, rootMatrix, svg)
    );
    inheritance.__scrollyLayerParentRootMatrix = inheritedMatrix;
  });
  localMatrices.forEach((matrix, layerId) => {
    const inheritance = inheritanceById.get(layerId);
    if (!inheritance) return;
    inheritance.style.transform = "";
    inheritance.setAttribute("transform", svgMatrixAttribute(matrix));
  });
};
  ...
  if (markupChanged || svgMissing) {
    host.dataset.svgReady = "false";
    host.hidden = true;
    host.innerHTML = namespaceSvgMarkup(markup, svgRuntimeNamespaceFor(host));
  ...
  const resolveVisualAnnotationTransitionDelay = function resolveVisualAnnotationTransitionDelay(
  frame,
  reducedMotion = false
) {
  if (
  ...
  return Math.max(0, Number(frame?.transitionDurationMs) || 0);
};
  const createVisualAnnotationTransitionGate = function createVisualAnnotationTransitionGate() {
  const pendingTransitions = new WeakMap();
  const cancel = (owner) => {
    const pending = pendingTransitions.get(owner);
    if (!pending) return;
    pending.frameWindow?.cancelAnimationFrame?.(pending.frameId);
    pending.frameWindow?.clearTimeout?.(pending.timerId);
    pendingTransitions.delete(owner);
  };
  ...
  const schedule = (
    ...
    cancel(owner);
    if (!owner || !frameWindow || typeof reveal !== "function") {
      if (isCurrent() && isPresented()) reveal?.();
      return;
    }
    ...
    const waitForPresentation = () => {
      ...
      if (!isPresented()) {
        if (typeof frameWindow.requestAnimationFrame !== "function") return;
        const frameId = frameWindow.requestAnimationFrame(waitForPresentation);
        pendingTransitions.set(owner, { frameWindow, frameId });
        return;
      }
      const duration = Math.max(0, Number(delayMs) || 0);
      if (duration === 0 || typeof frameWindow.setTimeout !== "function") {
        finish();
        return;
      }
      const timerId = frameWindow.setTimeout(finish, duration);
      pendingTransitions.set(owner, { frameWindow, timerId });
    };
    waitForPresentation();
  };
  ...
  const transitionGate = createVisualAnnotationTransitionGate();
  ...
  return function syncVisualAnnotations(
    ...
    conceal(activeIndex);
    section.dataset.annotationActiveIndex = String(activeIndex);
    transitionGate.schedule(section, {
      frameWindow,
      delayMs: resolveVisualAnnotationTransitionDelay(frame, reducedMotion),
      isCurrent: () =>
        Number(section.dataset.annotationActiveIndex) === activeIndex,
      isPresented: () => isCheckpointPresented(activeIndex),
      reveal: () => reveal(activeIndex)
    });
  };
};
  ...
  const resolveImageSequenceFrameIndex = (progress, frameCount) => {
  ...
  const drawImageSequenceFrame = (
  canvas,
  image,
  fit = "cover",
  pixelRatio = 1
) => {
  if (!canvas || !image?.naturalWidth || !image?.naturalHeight) return false;
  ...
  const imageSequenceSections = Array.from(
    document.querySelectorAll("[data-image-sequence-section]")
  );
  const imageSequenceMobileQuery = matchMedia("(max-width: 700px)");
  const IMAGE_SEQUENCE_CACHE_LIMIT = 16;
  const imageSequenceStates = imageSequenceSections.map((section) => {
    const manifestElement = section.querySelector("[data-image-sequence-manifest]");
    const canvas = section.querySelector(".story-image-sequence__canvas");
    const poster = section.querySelector(".story-image-sequence__poster");
    let manifest = {};
    try { manifest = JSON.parse(manifestElement?.textContent || "{}"); } catch {}
    return {
      section,
      canvas,
      poster,
  ...
  const imageSequenceVariant = (state) =>
    imageSequenceMobileQuery.matches && state.manifest.mobile
      ? state.manifest.mobile
      : state.manifest.desktop || state.manifest.mobile;
  const loadImageSequenceFrame = (state, variant, index) => {
    const avif = variant?.avif?.[index] || "";
    const webp = variant?.webp?.[index] || "";
    const key = avif + "|" + webp;
    if (!key || key === "|") return Promise.reject(new Error("Missing frame"));
    if (state.cache.has(key)) {
      const existing = state.cache.get(key);
      state.cache.delete(key);
      state.cache.set(key, existing);
      return existing;
    }
    const load = (url) => new Promise((resolve, reject) => {
      if (!url) { reject(new Error("Missing source")); return; }
    ...
    const pending = load(avif).catch(() => load(webp));
    state.cache.set(key, pending);
    while (state.cache.size > IMAGE_SEQUENCE_CACHE_LIMIT) {
      const oldest = state.cache.keys().next().value;
      if (!oldest) break;
      state.cache.delete(oldest);
    }
    pending.catch(() => state.cache.delete(key));
    return pending;
  };
  const drawRequestedImageSequenceFrame = (state, index) => {
    const variant = imageSequenceVariant(state);
    if (!state.canvas || !variant?.frameCount) return;
    state.requestedIndex = index;
    loadImageSequenceFrame(state, variant, index).then((image) => {
      if (state.requestedIndex !== index) return;
      if (drawImageSequenceFrame(
        state.canvas,
        image,
        state.section.dataset.imageSequenceFit || "cover",
        Math.min(2, devicePixelRatio || 1)
      )) {
        state.drawnIndex = index;
        state.canvas.classList.add("is-ready");
        state.poster?.setAttribute("aria-hidden", "true");
      }
    }).catch(() => {});
    for (let offset = 1; offset <= 3; offset += 1) {
      if (index - offset >= 0) loadImageSequenceFrame(state, variant, index - offset).catch(() => {});
      if (index + offset < variant.frameCount) loadImageSequenceFrame(state, variant, index + offset).catch(() => {});
    }
  };
  const updateImageSequences = () => {
    imageSequenceStates.forEach((state) => {
      if (!state.active || reducedMotion.matches) return;
      const variant = imageSequenceVariant(state);
      if (!variant?.frameCount) return;
      const rect = state.section.getBoundingClientRect();
      const stageHeight = state.section.querySelector(".story-image-sequence__stage")?.clientHeight || innerHeight;
      const travel = Math.max(1, rect.height - stageHeight);
      const progress = Math.min(1, Math.max(0, -rect.top / travel));
      const index = resolveImageSequenceFrameIndex(progress, variant.frameCount);
      if (index !== state.requestedIndex || state.drawnIndex < 0) {
        drawRequestedImageSequenceFrame(state, index);
      }
    });
  };
  if ("IntersectionObserver" in window) {
    const imageSequenceObserver = new IntersectionObserver((entries) => {
      entries.forEach((entry) => {
        const state = imageSequenceStates.find((candidate) => candidate.section === entry.target);
        if (state) state.active = entry.isIntersecting;
      });
      scheduleUpdate();
    }, { rootMargin: "100% 0px" });
    imageSequenceStates.forEach((state) => imageSequenceObserver.observe(state.section));
  }
  imageSequenceMobileQuery.addEventListener?.("change", () => {
    imageSequenceStates.forEach((state) => {
      state.cache.clear();
  ...
  const update = () => {
    ...
    panZoomSections.forEach((section) => {
      ...
      syncVisualAnnotations(
        ...
        (index) =>
          getPanZoomPresentationSnapshot(media).presentedIndex === index
      );
